 ...
 */
export function getInitialState(containerId) {
...
export function useTurboCache(containerId, currentState, autoRestore = true) {
  ...
  // Store current state in the container's data attribute before cache
  const handleBeforeCache = () => {
    try {
      const stateJson = JSON.stringify(currentState);
      container.dataset.initialState = stateJson;
    } catch (e) {
      console.warn('IslandJS Turbo: Failed to serialize state for cache', e);
    }
  };

  // Optional: Restore state on turbo:load (for components that need it)
  const handleTurboLoad = () => {
    if (autoRestore) {
      // This could trigger a re-render if the component listens for this event
      const event = new CustomEvent('turbo:state-restored', {
        detail: { containerId, state: getInitialState(containerId) }
      });
  ...
  const handleBeforeRender = () => {
    try {
      ...
      console.warn('IslandJS Turbo: Failed to serialize state for before-render', e);
    }
  };
